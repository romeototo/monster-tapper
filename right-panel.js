...
function loadRPState() {
  ...
  try {
    ...
    if (d.buffs)
      d.buffs.forEach((sb) => {
        const b = passiveBuffs.find((x) => x.id === sb.id);
        if (b) b.level = sb.level;
      });
    if (d.relics)
      d.relics.forEach((sr) => {
        const r = relicsData.find((x) => x.id === sr.id);
        if (r) r.owned = sr.owned;
      });
    if (d.pets)
      d.pets.forEach((sp) => {
        const p = petsData.find((x) => x.id === sp.id);
        if (p) {
          p.owned = sp.owned;
          p.active = sp.active;
        }
    ...
    if (d.achievements)
      d.achievements.forEach((sa) => {
        const a = achievementsData.find((x) => x.id === sa.id);
...
function buyBuff(b) {
  const cost = Math.floor(b.baseCost * Math.pow(b.costMult, b.level));
  if (typeof tokens === "undefined" || tokens < cost || b.level >= b.maxLvl)
    return;
  tokens -= cost;
  b.level++;
...
function checkAchievements() {
  ...
  achievementsData.forEach((a) => {
    ...
    if (
      a.type === "goldrush" &&
      typeof goldRushTriggered !== "undefined" &&
      goldRushTriggered >= a.target
    )
      met = true;
    if (
      a.type === "prestige" &&
      typeof ascensionCount !== "undefined" &&
      ascensionCount >= a.target
    )
      met = true;
    if (met) {
      a.unlocked = true;
...
// ─── ESSENCE TRACKING ────────────────────────
window.trackEssence = function (amount) {
  lifetimeEssence += amount;
};
