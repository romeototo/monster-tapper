...
if (bgCanvas) {
  ...
  const STAR_LAYERS = [
    ...
    { count: 55, speed: 0.14, size: 1.5, baseOpacity: 0.5 },
    { count: 22, speed: 0.28, size: 2.4, baseOpacity: 0.8 },
  ];
  const stars = [];
  ...
  document.addEventListener("click", (e) => {
    ...
    if (
      e.target.closest(
        ".info-panel, .upgrades-panel, .skills-container, .offline-modal",
      )
    )
      return;
    for (let i = 0; i < 2; i++) {
